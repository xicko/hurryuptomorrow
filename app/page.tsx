...
export default function Home() {
  ...
  const [dateCurrent, dateState] = useState('2025-01-24T00:00:00'); // countdown date state

  const toggleState = () => {
    textState1((prev) => (prev === textOn ? textOff : textOn)); // album
    textState2((prev) => (prev === textOff ? textOn : textOff)); // movie
    dateState((prev) => (prev === '2025-01-24T00:00:00' ? '2025-05-16T00:00:00' : '2025-01-24T00:00:00')); // toggle function between release dates
  };
    ...
    <section className='grid justify-center place-content-center w-full h-[100dvh] text-[#c01101] select-none'>
      ...
      <div className='flex justify-center py-2'>
        <div className='pt-2 flex flex-row gap-4 md:text-lg text-md'>
          <p className={`${textCurrent1} transition-all ease-in-out`}>
            Album
          </p>

          <p className={`${textCurrent2} transition-all ease-in-out`}>
            Movie
          </p>
        </div>
      </div>
      ...
      <div onClick={toggleState} className='fixed z-20 cursor-pointer inset-0 flex justify-center items-center'></div>
